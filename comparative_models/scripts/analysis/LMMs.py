...
# Create a scatter plot
plt.scatter(df['bdi'], df['avg_feedback_aligment'], color='blue', alpha=0.5, label='Data points')
...
lmm_model = smf.mixedlm(lmm_formula, df, groups=df["pid"], re_formula="1"
                        )
...
#%% LMM during baseline: Confidence ~ error

# Specify dataframe
df = expanded_df.copy()

# Group by 'pid' and 'session', then shift the 'performance'
# shift down by one to get 'previous_feedback' and 'previous_performance'

...
# Fit models to data from Experiment 1 - Fixed Feedback
import numpy as np
import pandas as pd
...
def process_session(df_s):
    ...
    confidence = df_s.confidence.values # Estimates from participant.
...
def main(df):
    ...
    print(f"Number of pid-session combos: {num_unique_combos}")
    ...
    with Pool(48) as pool:
        for sim_id, metrics in tqdm(pool.map(process_session, df_list),
            ...
            for key, value in metrics.items():
                print(key, idx, value)
                if key in ['pid', 'condition']:  # String fields
    ...
    # Relative save path
    relative_path = "../../results/Fixed_feedback/model_comparison/models_fit_to_data"

    # Construct the full path to the file
    file_path = os.path.normpath(os.path.join(script_dir, relative_path))
    file_name = r"model_fits_EXP1_data"
...
if __name__ == '__main__':
    ...
    # For debugging
    #df = df[df['pid']=='5aeb3c327ed5460001572a5e']

# -*- coding: utf-8 -*-
"""
Created on Thu Apr 10 15:07:51 2025
...
def process_session(df_s):
    ...
    df_s = df_s.copy()  # Copy df_s
    df_s['difference'] = abs(df_s['estimate'] - df_s['correct']) # Abs error
    abs_error_avg = df_s.groupby('trial')['difference'].mean() # Average
...
def main(df):
    ...
    for session_df in tqdm(df_list, total=len(df_list),
        ...
        for model_name in model_names:
            ...
            # by filtering on model name and removing '_conf'
            parameters = {k: v for k, v in session_results.items()
              if k.startswith(f"{model_name}_")  # precise match
            ...
            for trial_idx, (confidence, trial_num) in enumerate(zip(confidence_data, session_df['trial'])):
                trial_data = {
...
def plot_model_results_with_real_data(df, df_real):
    ...
    # Filter out baseline trials in real data
    df_real = df_real[df_real.condition != 'baseline']

    # Drop duplicates first outside the loop
    df_real = df_real.drop_duplicates(subset=['session', 'pid', 'trial'],
                                      keep='first')

    # Sort data to ensure trial order is preserved
    df_real = df_real.sort_values(by=['session', 'pid', 'trial'])

    # Reset trial numbers starting from 0 for each (session, pid) group
    df_real['trial'] = df_real.groupby(['session', 'pid']).cumcount()

    # Set up a 3-column subplot structure
    # (real data, individual simulations, mean confidence)
    ...
    for model_idx, model_name in enumerate(model_names):
        ...
        for (session_id, pid) in df_real[['session', 'pid']].drop_duplicates().itertuples(index=False):
            session_df_real = df_real[(df_real['session'] == session_id) &
                                      (df_real['pid'] == pid)]

            ax_real.plot(range(len(session_df_real['trial'])),
                         session_df_real['confidence'],
                         label=f'Session {session_id}, PID {pid}', marker='o',
                         color='C0', alpha=0.7)

        # Plot each session's confidence for the current model in the second column (simulations)
        for (session_id, pid) in model_df[['session', 'pid']].drop_duplicates().itertuples(index=False):
            session_df = model_df[(model_df['session'] == session_id) &
                                  (model_df['pid'] == pid)]

            ax_individual.plot(range(len(session_df['trial'])),
        ...
        ax_mean.plot(range(len(mean_confidence_real.index)), mean_confidence_real.values, color='black',
                     marker='o', linestyle='-', linewidth=2, label="Real")
        ax_mean.plot(range(len(mean_confidence.index)), mean_confidence.values, color='b',
    ...
    file_name = r"model_simulations_EXP1"
...
if __name__ == '__main__':
    ...
    # Reset trial numbers to feedback trials per pid-session-subtrial
    df['trial'] = df.groupby(['pid', 'session', 'subtrial']).cumcount()
